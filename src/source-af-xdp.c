...
static TmEcode AFXDPSocketCreation(AFXDPThreadVars *ptv,int xsk_map_fd)
{
    ...
    /* Open AF_XDP socket */
    if (OpenXSKSocket(ptv,xsk_map_fd) != TM_ECODE_OK) {
        SCReturnInt(TM_ECODE_FAILED);
    }
    ...
    if (bpf_xdp_query_id(ptv->ifindex, ptv->xsk.cfg.xdp_flags, &ptv->prog_id)) {
        SCLogError("Failed 1 to attach eBPF program to interface: %s  prog_id:%d", ptv->livedev->dev,ptv->prog_id);
        SCReturnInt(TM_ECODE_FAILED);
    }
#else
    if (bpf_get_link_xdp_id(ptv->ifindex, &ptv->prog_id, ptv->xsk.cfg.xdp_flags)) {
        SCLogError("Failed 2 to attach eBPF program to interface: %s, prog_id:%d", ptv->livedev->dev,ptv->prog_id);
        SCReturnInt(TM_ECODE_FAILED);
    }
#endif
